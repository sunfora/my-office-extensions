...
Доступные нам элементы: списки, кнопки, текст, чекбоксы и радио.
...
И затем мы добавляем эвенты к кнопкам, ну например для того чтобы обновить ListBox с помощью кнопок я делаю нечто подобное.
У меня есть состояние формы в приложении - `headers`, я обходя эти `headers` ререндерю список, просто через `ListBox:setItems()`.
Иначе говоря ui не полностью статичный, что-то в нём можно весьма неплохо динамически менять.
Но что-то другое как например лейаут самого диалога поменять не представляется возможным, у ui:Column просто нет никаких таких методов которые можно было бы поменять.
...
И сам диалог спавнится через `context.showDialog`. 

> Кстати говоря, в отличие от messageBox по какой-то причине `showDialog` не блокирующая операция, несмотря на то, что сам по себе диалог модальная вещь.
